...
from nemo.multi_view_sequence_jeff_exp import PennActionMultiViewSequence, MultiViewSequence, DemoMultiViewSequence
...
class MultiViewModel(nn.Module):
    ...
    def __init__(self, args, multi_view_seqs, device):
        ...
        #the intrinsic params are arrays of length num_views
        self.FOCAL_LENGTH = multi_view_seqs.FOCAL_LENGTH
        ...
        # Camera Params
        # extrinsic_rotmat, extrinsic_trans = self.unpack_json(self.args.droidslam)
        # self.extrinsic_rotmat = torch.tensor(extrinsic_rotmat).float()
        # self.extrinsic_trans = torch.tensor(extrinsic_trans).float()
        self.extrinsic_rotmat = multi_view_seqs.rotations_alt.float().to(device)
        self.extrinsic_trans = multi_view_seqs.translations_alt.float().to(device)
        # print("Loaded extrinsic rotmat: ", self.extrinsic_rotmat.shape)
        # print("Loaded extrinsic trans: ", self.extrinsic_trans.shape)
        self.extrinsic_trans_init = self.extrinsic_trans.clone().detach()
        ...
        # Renderer
        #TODO: figure out if renderer needs to be different for each view
        self.renderer = Renderer(focal_length=self.FOCAL_LENGTH[0],
                                 img_width=self.IMG_D1[0],
                                 img_height=self.IMG_D0[0],
        ...
        self.vibe_renderer = VIBERenderer(resolution=(self.IMG_D1[0],
                                                      self.IMG_D0[0]),
                                          orig_img=True,
                                          wireframe=False)
    ...
    # def unpack_json(self, directory_path):
    #     print("Debugging unpack_json: ", directory_path)
    #     json_files = [f for f in os.listdir(directory_path) if f.endswith('.json')]
    #     json_files.sort(key=lambda f: int(f.split('.')[-2]))
    ...
    #         rotations = [torch.tensor(np.array(r).reshape(3, 3)) for r in rotations]
    #         translations = [torch.tensor(np.array(t).reshape(3, 1)) for t in translations]

    #         all_rotations.append(rotations)
    #         all_translations.append(translations)

    #     # Determine the minimum length among all lists
    #     min_length = min(min(len(r) for r in all_rotations), min(len(t) for t in all_translations)) - 1
    #     print("Min length: ", min_length)

    #     # Trim each list in all_rotations and all_translations to the minimum length
    #     all_rotations = [rot[:min_length] for rot in all_rotations]
    #     all_translations = [trans[:min_length] for trans in all_translations]
    #     print("all_rotations: ", len(all_rotations))
    #     # Convert list of tensors to tensor
    #     all_rotations = torch.stack([torch.stack(rot) for rot in all_rotations])
    #     print("all_rotations final shape: ", all_rotations.shape)
    #     all_translations = torch.stack([torch.stack(trans) for trans in all_translations])
    ...
    #                     #view_idx, :3][None]
    #                 camera_translation = self.extrinsic_trans[view_idx]
    #                 #rot6d = self.learned_cameras[view_idx, 3:][None]
    #                 #camera_rotation = rot6d_to_rotmat(rot6d)
    #                 camera_rotation = self.extrinsic_rotmat[view_idx]
    ...
    #                     translation=camera_translation.squeeze()) #.expand(batch_size, -1))
    #                 # ipdb.set_trace()
    #                 im = renderer(
    #                     transformed_points3d[0].detach().cpu().numpy(),
    #                     np.zeros_like(
    #                         camera_translation[0][0].squeeze().detach().cpu().numpy()),
    #                     frame_im / 255.,
    #                     return_camera=False)
    ...
    def render_rollout_mv_figure(self,
        ...
        for ridx in range(nrow):
            ...
            for cidx in range(ncol):
                ...
                # Prepare camera intrinsics (fixed)
                focal_length = torch.ones(1) * self.FOCAL_LENGTH[view_idx]
                camera_center = torch.ones(1, 2).to(self.device)
                camera_center[0, 0] = self.IMG_D0[view_idx] // 2
                camera_center[0, 1] = self.IMG_D1[view_idx] // 2
                ...
                frame_im = np.ones((self.IMG_D0[view_idx], self.IMG_D1[view_idx], 3))
    ...
    def render_rollout_figure(self,
        ...
        for ridx in range(nrow):
            ...
            for cidx in range(ncol):
                ...
                camera_translation = self.extrinsic_trans[view_idx][frame_idx]
                #rot6d = self.learned_cameras[view_idx, 3:][None]
                #camera_rotation = rot6d_to_rotmat(rot6d)
                camera_rotation = self.extrinsic_rotmat[view_idx][frame_idx]
                ...
                points3d = pose[None]
                print("Debugging render rollout video------------")
                print("shape of points3d: ", points3d.shape)
                print("shape of camera rotation: ", camera_rotation.shape)
                print("shape of camera translation: ", camera_translation.shape)

                batch_size = points3d.shape[0]
                transformed_points3d = apply_extrinsics(
                    points3d,
                    rotation=camera_rotation.expand(batch_size, -1, -1),
                    translation=camera_translation.squeeze().expand(batch_size, -1)) #.expand(batch_size, -1))
                ...
                im = renderer(
                    transformed_points3d[0].detach().cpu().numpy(),
                    np.zeros_like(
                        camera_translation[0].squeeze().detach().cpu().numpy()),
                    frame_im,
                    return_camera=False)
    ...
    def render_pretty_rollout_figure(self,
        ...
        for ridx in range(nrow):
            ...
            im = renderer(vertices,
                          camera_rotation[0][0].detach().cpu().numpy(),
                          np.array([0, 0, 30]),
                          return_camera=False,
                          view_type=view_type,
    ...
    def render_pretty_individual_figure(self,
        ...
        camera_translation = camera_translation[0][0].squeeze().detach().cpu().numpy()
    ...
    def humor_loss(self, pred_dict):
        ...
        print("latent_motion_pred shape: ", latent_motion_pred.shape)
        ...
        trans_vel, joints_vel, root_orient_vel = \
            ...
            pred_dict['trans'], pred_dict['orient_aa'], body_pose, beta, self.data_fps)
        print("trans_vel shape: ", trans_vel.shape)
        print("joints_vel shape: ", joints_vel.shape)
        print("root_orient_vel shape: ", root_orient_vel.shape)
        
        trans = pred_dict['trans'][:,:1]
        root_orient = pred_dict['orient_aa'][:,:1]
        body_pose = body_pose[:,:1]
        trans_vel = trans_vel[:,:1]
        root_orient_vel = root_orient_vel[:,:1]
        joints_vel = joints_vel[:,:1]

        prior_opt_params = [trans_vel, joints_vel, root_orient_vel]
        rollout_results, _ = self.motion_optimizer.rollout_latent_motion(
            trans,  root_orient, body_pose, beta, prior_opt_params, latent_motion_pred, return_prior=True)
        print("rollout_results first element shape: ", rollout_results['cond_prior'][0].shape)
        print("rollout_results second element shape: ", rollout_results['cond_prior'][1].shape)
    ...
    def keypoint_loss(self,
        ...
        """ 
        """
        print("shape of pred_keypoints_2d: ", pred_keypoints_2d.shape)
        print("shape of gt_keypoints_2d: ", gt_keypoints_2d.shape)
    ...
    def opt_cam(self, cam_opt_steps=2000):
        ...
        def closure():  # LBFGS needs a closure
            ...
            # Project to 2D
            points2d = self.learned_camera_projection_old(j, view_idx)
        ...
        #print("DEBUGGING HYBRID CAM ------------------------------------")
        # detached_learned_cameras = self.learned_cameras.detach()
        # camera_translation = detached_learned_cameras[:, :3][None]
        # rot6d = detached_learned_cameras[:, 3:][None]
        # camera_rotation = rot6d_to_rotmat(rot6d)
        # # print("applying learned ones to DEBUG")
        # # print("size of camera_translation: ", camera_translation.shape)
        # # print("size of camera_rotation: ", camera_rotation.shape)
        # #camera_rotation = camera_rotation.unsqueeze(1)
        # camera_translation = camera_translation.permute(1, 2, 0)
        # print("shape of camera_translation: ", camera_translation.shape)
        # print("shape of camera_rotation: ", camera_rotation.shape)
        # # self.extrinsic_rotmat = camera_rotation.expand(-1, self.extrinsic_rotmat.size(1), -1, -1)
        # # self.extrinsic_trans = camera_translation.expand(-1, self.extrinsic_rotmat.size(1), -1, -1)
        # # print("self.extrinsic_rotmat: ", self.extrinsic_rotmat)
        # # print("self.extrinsic_trans: ", self.extrinsic_trans)
        # self.extrinsic_rotmat = [camera_rotation]
        # self.extrinsic_trans = [camera_translation]

        # print("number of elements in rotmat_diff: ", self.rotmat_diff.shape[1])

        # for i in range(self.rotmat_diff.shape[1]):
        #     rotation_diff = self.rotmat_diff[:,i]
        #     translation_diff = self.trans_diff[:,i]
        #     print("shape of rotation_diff: ", rotation_diff.shape)
        #     print("shape of translation_diff: ", translation_diff.shape)

        #     # Update rotation and translation
        #     camera_rotation = camera_rotation + rotation_diff  # Multiply the current rotation matrix with the difference rotation matrix
        #     camera_translation = camera_translation + translation_diff  # Add the translation difference to the current translation

        #     self.extrinsic_rotmat.append(camera_rotation)
        #     self.extrinsic_trans.append(camera_translation)

        # # Convert lists to tensors
        # self.extrinsic_rotmat = torch.stack(self.extrinsic_rotmat, dim = 1)
        # self.extrinsic_trans = torch.stack(self.extrinsic_trans, dim = 1)
        # self.extrinsic_trans_init = self.extrinsic_trans.clone()
        # print("shape of self.extrinsic_rotmat: ", self.extrinsic_rotmat.shape)
        # print("shape of self.extrinsic_trans: ", self.extrinsic_trans.shape)
    ...
    #         # print the dimension of camera_translation
    #         print('dimension for camera_translation: ', camera_translation.shape)
    #         rot6d = self.learned_cameras[cur_view_idx, 3:][None]
    #         camera_rotation = rot6d_to_rotmat(rot6d)
    #         # print the dimension of camera_rotation
    #         print('dimension for camera_rotation: ', camera_rotation.shape)
    ...
    #     return out
    def reproject(self, points3d, cam_R, cam_t, cam_f, cam_center):
        """
    ...
    def learned_camera_projection(self, input_points3d, view_idx):
        ...
        original_item_idx = []
        points3d = []
        print("shape of input_points3d", input_points3d.shape)
        print("shape of view_idx", view_idx.shape)
        ...
        for cur_view_idx in range(self.num_views):
            ...
            camera_translation = self.extrinsic_trans[cur_view_idx]
            # print the dimension of camera_translation
            print('dimension for camera_translation: ', camera_translation.shape)
            #rot6d = self.learned_cameras[cur_view_idx, 3:][None]
            camera_rotation = self.extrinsic_rotmat[cur_view_idx]
            # print the dimension of camera_rotation
            print('dimension for camera_rotation: ', camera_rotation.shape)

            # Prepare camera intrinsics (fixed)
            focal_length = torch.ones(1, 2) * self.FOCAL_LENGTH[cur_view_idx]
            focal_length = focal_length.to(self.device)
            camera_center = torch.ones(1, 2).to(self.device)
            camera_center[0, 0] = self.IMG_D0[cur_view_idx] // 2
            camera_center[0, 1] = self.IMG_D1[cur_view_idx] // 2
            ...
            #     focal_length=focal_length.expand(batch_size),
            #     camera_center=camera_center.expand(batch_size, -1))
            pred_keypoints_2d = self.reproject(points3d[cur_view_idx].unsqueeze(0),
                                            camera_rotation.unsqueeze(0), 
                                            camera_translation.squeeze().unsqueeze(0),
                                            focal_length.expand(batch_size, -1), 
                                            camera_center.expand(batch_size, -1))
            pred_keypoints_2d = pred_keypoints_2d.squeeze(0)
            output_2d_dict[cur_view_idx] = pred_keypoints_2d
            print("shape of pred_keypoints_2d", pred_keypoints_2d.shape)
        D1 = pred_keypoints_2d.shape[1]
        D2 = pred_keypoints_2d.shape[2]
        ...
        for cur_view_idx in range(self.num_views):
            if len(points3d[cur_view_idx]) == 0:
                continue
            print("shape of original_item_idx[cur_view_idx]", original_item_idx[cur_view_idx].shape)
            out[original_item_idx[cur_view_idx]] = output_2d_dict[cur_view_idx]
            
        return out
    
    def learned_camera_projection_old(self, input_points3d, view_idx):
        """ 
        Input
            input_points3d -- (N_batch, D)
            view_idx       -- (N_batch,)  the value is which view a point is from.
        ...
        for cur_view_idx in range(self.num_views):
            ...
            # Prepare camera intrinsics (fixed)
            focal_length = torch.ones(1) * self.FOCAL_LENGTH[0]
            camera_center = torch.ones(1, 2).to(self.device)
            camera_center[0, 0] = self.IMG_D0[0] // 2
            camera_center[0, 1] = self.IMG_D1[0] // 2
...
class NemoV1(MultiViewModel):
    ...
    def __init__(self, args, multi_view_seqs, device):
        ...
        camera_init = torch.zeros(2, self.num_views, 9).to(device)
        camera_init = 1e-4 * torch.randn(self.num_views, 9).to(device)
        camera_scale = torch.ones(self.num_views, 1, 1, 1).to(device)
        self.camera_scales = nn.Parameter(camera_scale)
        self.learned_cameras = nn.Parameter(camera_init)
        # Init the camera depth
        self.learned_cameras.data[..., 3] += 1
        self.learned_cameras.data[..., 6] += 1
        self.learned_cameras.data[
            ..., 2] += 2 * self.FOCAL_LENGTH[0] / (self.IMG_D0[0] * 1 + 1e-9)
        ...
        self.opt_camera_scales = torch.optim.Adam(params=[self.camera_scales],
                                                 lr=self.args.lr_camera,
                                                 weight_decay=0)
        ...
        self.optimizers = [self.opt_cameras, self.opt_camera_scales, self.opt_motion, self.opt_phase]
...
class NemoV2KnownCam(NemoV2):
    """
    V2 + known camera
    """

    def __init__(self, args, multi_view_seqs, device):
        super(NemoV2KnownCam, self).__init__(args, multi_view_seqs, device)
        print("Initializing NemoV2KnownCam.")

        # del self.learned_cameras
        # del self.opt_cameras
        self.optimizers = [self.opt_motion, self.opt_phase]
        self.start_global_traj_anywhere = True


        # Load GT 3D
        # action = osp.split(self.args.nemo_cfg['exp_dir'])[1]
        # OPENCAP_ROOT = '/home/groups/syyeung/wangkua1/data/OpenCapData_03284efb-2244-4a48-aec9-abc34afdffc8'
        # trc_file = osp.join(OPENCAP_ROOT, 'MarkerData', f'{action}.trc')
        # cols = [f"{a}{b}" for a, b in product(['X', 'Y', 'Z'], range(1, 64))]
        # df = pd.read_csv(trc_file,
        #                  skiprows=[0, 1, 2, 3],
        #                  sep='\t',
        #                  usecols=cols)
        # data = np.array(df)
        # data = data.reshape(data.shape[0], 63, 3)
        # data[:, :, 1] -= 0.28
        # self.gt3d = to_tensor(data)

    # def opt_cam(self, cam_opt_steps=2000):
    #     return []

    # def my_project_point(self, points3d, R, tvec, mtx):
    #     # Flatten the first 2 dims
    #     A, B, _ = points3d.shape
    #     points3d = points3d.reshape(A * B, 3)

    #     # Prepare points3d
    #     N = points3d.shape[0]
    #     X = torch.cat([points3d, torch.ones((N, 1)).cuda()], 1).T  # (4, N)

    #     # Prepare camera matrix
    #     K = mtx
    #     E = torch.cat([R, tvec.T], 1)
    #     P = K @ E

    #     # Project
    #     x_prime = (P @ X).T
    #     x = x_prime[:, :2] / x_prime[:, 2:3]
    #     x = x.reshape(A, B, 2)
    #     return x
    def my_project_point(self, points3d, R, tvec, mtx):
        ...
        # Prepare a list to store all projected points
        all_projected_points = []
        
        # Iterate over each frame
        for i in range(num_frames):
            # Extract the points for this frame
            points3d_frame = points3d[i]  # Shape: (25, 3)
            
            # Prepare points3d
            N = points3d_frame.shape[0]
            X = torch.cat([points3d_frame, torch.ones((N, 1)).cuda()], 1).T  # (4, N)

            # Get the rotation and translation for this frame
            R_frame = R[i]  # Shape: (3, 3)
            tvec_frame = tvec[i]  # Shape: (3, 1)
            
            # Combine rotation and translation
            E = torch.cat([R_frame, tvec_frame], 1)
            
            # Compute the projection matrix for this frame
            P = K @ E
            ...
            # Append the projected points for this frame
            all_projected_points.append(x)
        
        # Stack all projected points
        all_projected_points = torch.stack(all_projected_points)  # Shape: (num_frames, 25, 2)
    ...
    #         # Get the rotation and translation for this frame
    #         R_frame = R[i].T  # Taking transpose for inverse rotation
    #         tvec_frame = -tvec[i]  # Negating the vector for inverse translation
    ...
    # def apply_extrinsics(self, points3d, R, tvec):
    #     # Flatten the first 2 dims
    #     A, B, _ = points3d.shape
    #     points3d = points3d.reshape(A * B, 3)
    ...
    def apply_extrinsics(self, points3d, R, tvec):
        ...
        # Prepare a list to store all transformed points
        ...
        for i in range(num_frames):
            ...
            # Append the transformed points for this frame
            all_transformed_points.append(x)
        
        # Stack all transformed points
        all_transformed_points = torch.stack(all_transformed_points)  # Shape: (num_frames, 25, 3)
    ...
    def learned_camera_projection(self, input_points3d, view_idx):
        ...
        for cur_view_idx in range(self.num_views):
            ...
            # Prepare camera extrinsics (learned)
            # cur_camera_dict = self.multi_view_seqs.gt_camera_dicts[
            #     cur_view_idx]
            # camera_translation = cur_camera_dict['translation']
            # camera_translation = torch.tensor(camera_translation.T).float().to(
            #     self.device)
            # camera_rotation = cur_camera_dict['rotation']
            # camera_rotation = torch.tensor(camera_rotation).float().to(
            #     self.device)
            ...
            # Prepare camera intrinsics (fixed)
            # intrinsic_mat = cur_camera_dict['intrinsicMat']
            # intrinsic_mat = to_tensor(intrinsic_mat)
            intrinsic_mat = torch.zeros(3, 3).to(self.device)
            intrinsic_mat[0, 0] = self.FOCAL_LENGTH[cur_view_idx]
            intrinsic_mat[1, 1] = self.FOCAL_LENGTH[cur_view_idx]
            intrinsic_mat[0, 2] = self.IMG_D1[cur_view_idx] / 2
            intrinsic_mat[1, 2] = self.IMG_D0[cur_view_idx] / 2
            intrinsic_mat[2, 2] = 1

            batch_size = len(points3d[cur_view_idx])
            points2project = points3d[cur_view_idx]
            points2project = points2project[..., [2, 1, 0]]
            points2project[:, 0] *= -1000
            ...
            #     focal_length=focal_length.expand(batch_size),
            #     camera_center=camera_center.expand(batch_size, -1))
            focal_length = torch.ones(1) * self.FOCAL_LENGTH[cur_view_idx]
            camera_center = torch.ones(1, 2).to(self.device)
            camera_center[0, 0] = self.IMG_D0[cur_view_idx] // 2
            camera_center[0, 1] = self.IMG_D1[cur_view_idx] // 2
            pred_keypoints_2d = perspective_projection(points2project,
                                                      rotation=camera_rotation,
                                                      translation=camera_translation.squeeze(),
                                                      focal_length=focal_length.expand(batch_size),
                                                      camera_center=camera_center.expand(batch_size, -1))
    ...
    def render_rollout_figure(self,
        ...
        for ridx in range(nrow):
            ...
            for cidx in range(ncol):
                ...
                # cur_camera_dict = self.multi_view_seqs.gt_camera_dicts[
                #     view_idx]
                # camera_translation = cur_camera_dict['translation'].copy(
                # )  #  [[0, 1, 2]]
                # # camera_translation[0] *
                # # = -1
                # camera_translation = torch.tensor(
                #     camera_translation.T).float().to(self.device)
                camera_translation = self.extrinsic_trans[view_idx]
                # camera_rotation = cur_camera_dict['rotation']
                # ipdb.set_trace()
                # rvec = cur_camera_dict['rotation_EulerAngles'].copy()  #/  2
                # # rvec = cur_camera_dict['rotation_EulerAngles'].copy() / (np.pi / 2)
                # # rvec[0] *= -1
                ...
                #intrinsic_mat = cur_camera_dict['intrinsicMat']
                intrinsic_mat = torch.zeros(3, 3).to(self.device)
                intrinsic_mat[0, 0] = self.FOCAL_LENGTH[view_idx]
                intrinsic_mat[1, 1] = self.FOCAL_LENGTH[view_idx]
                intrinsic_mat[0, 2] = self.IMG_D1[view_idx] / 2
                intrinsic_mat[1, 2] = self.IMG_D0[view_idx] / 2
                intrinsic_mat[2, 2] = 1
                focal_length = torch.ones(1).to(self.device) * intrinsic_mat[0, 0]
                focal_length = focal_length.to(self.device)  #* .5
                camera_center = torch.ones(1, 2).to(self.device)
                camera_center[0, 0] = intrinsic_mat[0, 2]  # // 2
                camera_center[0, 1] = intrinsic_mat[1, 2]  # // 2
                """
                array([[913.22246674,   0.        , 366.48600693],
                       [  0.        , 913.52421052, 638.50156427],
                       [  0.        ,   0.        ,   1.        ]])
                """
                pose = pred_dict['v'][view_idx, frame_idx]
                points3d = pose[None]

                points2project = points3d
                # points2project = points2project[..., [2, 1, 0]]
                # points2project *= -1000
                """ Uses Pyrender to apply extrinsics... doesn't work 
                hint: https://stackoverflow.com/questions/18637494/camera-position-in-world-coordinate-from-cvsolvepnp/18643735#18643735
                transformed_points3d = points2project
                if no_bg:
                    frame_im = np.ones_like(frame_im)
                else:
                    frame_im = frame_im / 255.
                ...
                transformed_points3d = self.apply_extrinsics(
                    points2project, camera_rotation, camera_translation)
                if no_bg:
                    frame_im = np.ones_like(frame_im)
                else:
                    frame_im = frame_im / 255.
                ...
                im = renderer(
                    transformed_points3d[0].detach().cpu().numpy(),
                    np.zeros_like(
                        camera_translation[0][0].detach().cpu().numpy()),
                    frame_im,
                ...
                # Add vertices
                #joints3d = self.gt3d[frame_idx]
                joints3d = pred_dict['v'][view_idx, frame_idx]
                # Project to 2D
                joints2d = self.learned_camera_projection(
                    joints3d[None],
                    torch.tensor([view_idx]).int().cuda())[0]
                joints2d = joints2d.detach().cpu().numpy()
                for joint_index in range(0, len(joints2d), 1):
                    im = cv2.circle(im,
                                    joints2d[joint_index, :2].astype('int32'),
                                    radius=5,
                                    color=[00, 00, 00],
                                    thickness=-1)

                # Add keypoints
                joints3d = pred_dict['j'][view_idx, frame_idx]
                # Project to 2D
                joints2d = self.learned_camera_projection(
                    joints3d[None],
                    torch.tensor([view_idx]).int().cuda())[0]
                joints2d = joints2d.detach().cpu().numpy()
                cs = [
                    'r', 'r', 'r', 'g', 'g', 'r', 'g', 'g', 'b', 'b', 'm', 'm',
                    'b', 'm', 'm', 'w', 'w', 'w', 'w', 'k', 'k', 'k', 'k', 'k',
                    'k'
                ]
                #
                for joint_index in range(len(joints2d)):
                    # c = joint_index % 10
                    # ipdb.set_trace()
                    o = cs[joint_index]
                    im = cv2.circle(
                        ...
                        joints2d[joint_index, :2].astype('int32'),
                        radius=5,
                        color=[int(255 * (v * .5)) for v in colors.to_rgb(o)],
                        thickness=-1)
...
class NemoV3(NemoV2):
    ...
    def _step(self, view_idx, frame_idx, update=True, full_batch=False):
        """
        Input
            view_idx -- list of view ids
            frame_idx -- list of frame ids
        ...
        # Project to 2D
        points2d = self.learned_camera_projection_old(j,
                                                  view_idx)  # (N_batch, 25, 2)
    ...
    def step(self, view_idx, frame_idx, update=True, full_batch=False):
        ...
        #extrinsic_trans_copy = self.extrinsic_trans_init.clone().detach()
        # self.extrinsic_trans = self.extrinsic_trans_init * self.camera_scales
        #print("extrinsic_trans_copy: ", extrinsic_trans_copy)
        # print("camera scales: {}".format(self.camera_scales))
        # print("self.extrinsic_trans after scaling: ", self.extrinsic_trans)
        #print("self.learned_cameras: ", self.learned_cameras)
