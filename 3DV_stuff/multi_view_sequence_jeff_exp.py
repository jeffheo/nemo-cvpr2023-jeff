...
def prepare_person_dict(person_output, max_frames):
    ...
    for key in person_output:
        ...
        print("person_output['frame_ids']:", person_output['frame_ids'].shape)
...
# def unpack_json(directory_path):
...
#         intrinsics = [int(intrinsics[0][i]) for i in [0, 4, 5]] #focal, D1, D0
...
#         # Compute differences between each adjacent tensor for rotations and translations
#         rotations_diff = [rotations[i+1] - rotations[i] for i in range(len(rotations)-1)]
#         translations_diff = [translations[i+1] - translations[i] for i in range(len(translations)-1)]

#         all_rotations.append(rotations_diff)
#         all_translations.append(translations_diff)
#         all_intrinsics.append(intrinsics)

#     return all_rotations, all_translations, all_intrinsics
...
def load_data(cam_dir, action):
    # camera info
    sidx = 0
    fpath = os.path.join(cam_dir, f"{action}.npz")
    print("fpath: ", fpath)
...
#             t0 = -cam_t[sidx:eidx].mean(dim=0) + torch.randn(3) * 0.1
        ...
        cam_t = cam_t - t0
    return cam_R, cam_t, intrins

def unpack_json(directory_path, names):
    print("Debugging unpack_json: ", directory_path)
    json_files = [f for f in os.listdir(directory_path) if f.endswith('.json') and f[:-5] in names]
    json_files.sort(key=lambda f: int(f.split('.')[-2]))
    ...
    for json_file in json_files:
        ...
        # Convert lists into numpy arrays for easier manipulation
        # rotations = [torch.einsum("ij,jk->ik", T, torch.tensor(np.array(r).reshape(3, 3), dtype=torch.float32)) for r in rotations]
        # translations = [torch.einsum("ij,jk->ik", T, torch.tensor(np.array(t).reshape(3, 1), dtype=torch.float32)) for t in translations]
        rotations = [torch.tensor(np.array(r).reshape(3, 3), dtype=torch.float32) for r in rotations]
        translations = [torch.tensor(np.array(t).reshape(3, 1), dtype=torch.float32) for t in translations]
...
class DemoMultiViewSequence:
    ...
    def __init__(self, nemo_data_cfg, start_phase, num_frames, droidslam):
        self.nemo_data_cfg = nemo_data_cfg
        self.start_phase = start_phase
        self.sequences = []
        self.droidslam = droidslam
        ...
        #self._rotations, self._translations, self.intrinsics = unpack_json(self.droidslam, self.nemo_data_cfg['videos']['names'])
        self.rotations, self.translations = [], []
        #print("self._rotations: ", len(self._rotations[0]))

        self.cam_R, self.cam_t, self.intrins = load_data(self.droidslam, self.nemo_data_cfg['videos']['names'][0])

        print("self.cam_R: ", self.cam_R.shape)
        print("self.cam_t: ", self.cam_t.shape)
        print("self.intrins: ", self.intrins.shape)
        ...
        for view_idx, name in enumerate(self.nemo_data_cfg['videos']['names']):
            ...
            for frame_idx in range(self.num_frames):
                ...
                idxes.append(tidx)
                # print("rotation: ", self._rotations[view_idx][tidx])
                # print("translation: ", self._translations[view_idx][tidx])

                # rotation.append(self._rotations[view_idx][tidx])
                # translation.append(self._translations[view_idx][tidx])
            ...
            # rotations_diff = [rotation[i+1] - rotation[i] for i in range(len(rotation)-1)]
            # translations_diff = [translation[i+1] - translation[i] for i in range(len(translation)-1)]
            #self.original_droidslam = torch.stack(translation)

            self.sequences.append(cur_seq)
            # self.rotations.append(rotation)
            # self.translations.append(translation)

            # self.IMG_D0.append(self.intrinsics[view_idx][2])
            # self.IMG_D1.append(self.intrinsics[view_idx][1])
            # self.FOCAL_LENGTH.append(self.intrinsics[view_idx][0])

            self.rotations_alt = self.cam_R[idxes].unsqueeze(0)
            self.translations_alt = self.cam_t[idxes].unsqueeze(0).unsqueeze(-1)
            self.original_droidslam = self.cam_t[idxes].unsqueeze(-1)
            print("self.rotations_alt: ", self.rotations_alt.shape)
            print("self.translations_alt: ", self.translations_alt.shape)
            print("D0 shape: ", self.intrins[0][3])
            print("D1 shape: ", self.intrins[0][2])
            print("Focal length: ", self.intrins[0][0])
            self.IMG_D0.append(int(self.intrins[0][3]) * 2) #height is D0
            self.IMG_D1.append(int(self.intrins[0][2]) * 2) #width is D1
            self.FOCAL_LENGTH.append(int(self.intrins[0][0]))
        ...
        # self.rotations = torch.stack([torch.stack(lst) for lst in self.rotations])
        # self.translations = torch.stack([torch.stack(lst) for lst in self.translations])
        # print("rotations shape: ", self.rotations.shape)
        # print("translations shape: ", self.translations.shape)
    ...
    def get_image(self, view_idx, frame_idx):
        ...
        frame_im = np.zeros((self.IMG_D0[view_idx], self.IMG_D1[view_idx], 3))
...
class MultiViewSequence:
    ...
    def __init__(self, nemo_data_cfg, start_phase, num_frames, droidslam, run_hmr=True):
        self.nemo_data_cfg = nemo_data_cfg
        self.start_phase = start_phase
        self.sequences = []
        self.droidslam = droidslam
        ...
        self.framerate_multiplier = []
        #self._rotations, self._translations, self.intrinsics = unpack_json(self.droidslam)
        self.rotations, self.translations = [], []
        ...
        for view_idx, name in enumerate(self.nemo_data_cfg['videos']['names']):
            ...
            self.sequences.append(cur_seq)
            # self.rotations.append(rotations_diff)
            # self.translations.append(translations_diff)
            self.IMG_D0.append(self.intrinsics[view_idx][2])
            self.IMG_D1.append(self.intrinsics[view_idx][1])
            self.FOCAL_LENGTH.append(self.intrinsics[view_idx][0])
...
class PennActionMultiViewSequence:
    ...
    def __init__(self, nemo_cfg, start_phase, num_frames, droidslam):
