...
class DemoMultiViewSequence:
    ...
    def __init__(self, nemo_data_cfg, start_phase, num_frames, droidslam):
        ...
        for view_idx, name in enumerate(self.nemo_data_cfg['videos']['names']):
            ...
            self.rotations = self.cam_R[idxes].unsqueeze(0)
            self.translations = self.cam_t[idxes].unsqueeze(0).unsqueeze(-1)
            self.original_droidslam = self.cam_t[idxes].unsqueeze(-1)
            print("shape of self.rotations: ", self.rotations.shape)
            print("shape of self.translations: ", self.translations.shape)
            print("D0: ", self.intrins[0][3])
            print("D1: ", self.intrins[0][2])
