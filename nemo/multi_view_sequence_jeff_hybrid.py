...
class DemoMultiViewSequence:
    ...
    def __init__(self, nemo_data_cfg, start_phase, num_frames, droidslam):
        ...
        self.cam_R, self.cam_t, self.intrins = load_data(self.droidslam, self.nemo_data_cfg['videos']['names'][0]) # dim begins with num_frames
        ...
        for view_idx, name in enumerate(self.nemo_data_cfg['videos']['names']):
            ...
            rotations_diff = self.cam_R[idxes] #.unsqueeze(0)
            translations_diff = self.cam_t[idxes] #.unsqueeze(0).unsqueeze(-1)
            self.original_droidslam = self.cam_t[idxes].unsqueeze(-1)

            self.rotations_diff = rotations_diff[1:] - rotations_diff[:-1] #dimension is num_frames - 1, 3, 3
            self.translations_diff = translations_diff[1:] - translations_diff[:-1] #dimension is num_frames - 1, 3
