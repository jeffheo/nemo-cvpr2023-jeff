...
from nemo.multi_view_sequence_jeff_slam import PennActionMultiViewSequence, MultiViewSequence, DemoMultiViewSequence
...
class MultiViewModel(nn.Module):
    ...
    def __init__(self, args, multi_view_seqs, device):
        ...
        # Camera Params
        self.extrinsic_rotmat = multi_view_seqs.rotations.float().to(device)
        self.extrinsic_trans = multi_view_seqs.translations.float().to(device)
    ...
    # LEARNED CAMERA PROJECTION BEFORE MOVING CAMERA IMPLEMENTATION
    ...
    def learned_camera_projection_old(self, input_points3d, view_idx): #this is just for camera optimization because it works with learned camera
        """ 
        Input
            input_points3d -- (N_batch, D)
            view_idx       -- (N_batch,)  the value is which view a point is from.
