...
expname=${cur_fname}-${action}-${lr_human}-${lr_instance}-${instance_code_size}-${lr_phase}-${weight_gmm_loss}-${weight_vp_loss}-${weight_vp_z_loss}-${h_dim}-${phase_rbf_dim}-${rbf_kernel}-${warmup_step}-${lr_factor}-${loss}-${label_type}-${weight_3d_loss}-${weight_humor_loss}
...
conda init bash
conda activate /pasteur/u/jeffheo/miniconda3/envs/nemo_jeff
