...
for weight_humor_loss in 1.2e-2; do
...
expname=${cur_fname}-${action}-${index}-${lr_human}-${lr_instance}-${instance_code_size}-${lr_phase}-${weight_gmm_loss}-${weight_vp_loss}-${weight_vp_z_loss}-${h_dim}-${phase_rbf_dim}-${rbf_kernel}-${warmup_step}-${lr_factor}-${loss}-${label_type}-${weight_3d_loss}-${weight_humor_loss}


cmd="python -m scripts.learned_multi_view_recon_nn_jeff_vf \
    --default_config config/default-v1.yml \
    --label_type ${label_type} \
    --data_loader_type demo \
    --nemo_cfg_path nemo/config/3dv/nemo-config.yml \
    --out_dir shared_out/jeff_out/moving_slam/${expname} \
    --lr_factor ${lr_factor} \
    --batch_size 512 \
    --n_steps ${n_steps} \
...
sbatch <<< \
"#!/bin/bash
#SBATCH --job-name=${cur_fname}-pasteur
#SBATCH --output=../slurm_logs/jeff/${cur_fname}-pasteur-%j-out.txt
#SBATCH --error=../slurm_logs/jeff/${cur_fname}-pasteur-%j-err.txt
#SBATCH --mem=64gb
